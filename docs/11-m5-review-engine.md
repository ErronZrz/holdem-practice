...
2. **Bot 对比**：以 `HeuristicStrategy(seed=0)` 在同一局面 `choose_action` 的结果作为「合理打法」参照。

参考 Bot **固定为 heuristic**（即使对局当时选用 `random`）：heuristic 才是「合理、不轻易被 exploit」的参照，随机策略无参考价值。
...
|---|---|---|---|
| `bad_call` | 面对下注跟注且 `equity < pot_odds` | 全街 | error |
| `bad_fold` | 面对下注弃牌且 `equity > pot_odds + 0.10` | 全街 | warning |
| `value_missed` | 无人下注过牌且 `equity >= 0.70` | 翻牌后 | warning |
...
- 「价值下注 / 纯空气」阈值与 `heuristic.py` 保持一致，便于与 Bot 对比相互印证。
...
- **仍为「vs 随机」而非「vs 对手范围」**：胜率估算与打牌时的口径一致，面对下注时仍会偏乐观。因此 `bad_call`（跟注赔率不足）的判据较严格，`bad_fold` 加了 `+0.10` 容差。范围建模留待后续（方向 B / CFR）。
