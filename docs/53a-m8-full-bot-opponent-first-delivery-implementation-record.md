...
| `test_mixed_bot_benchmark.py` | 计划槽 128（125 可行动 + 3 结构性不适用）；36 格与 278/277 配额合计 10000；补充 240 次；对抗 4224 手与 256 动作上限；阶段 A 排期 64 手且固定单一 seed / 单一风格 / 单一轮换；两 arm 使用同一牌堆派生流；缺清单 / 缺允许阶段 / 阶段 B 缺前置回执均被拒；输出目录不得覆盖；节点拒绝结构性不适用人数与未知类别；节点确定性回放；行为收集有界 |
...
- 阶段 A/B 的 runner 已实现但**尚未被执行过**，因此其端到端可运行性仍未经实测；首次执行仍须冻结逐张牌 fixture 清单、提供输出目录并单独获得运行许可。
- 未冻结任何逐张牌 fixture：`mixed_bot_states.py` 只提供 schema 与回放入口，不内置具体牌面。
...
- 本轮改动是**首期实现与常规回归**，不是质量验证，也不构成任何强度、均衡或生产可用证据。
- 不得把通过的 790 条测试表述为对手质量、不可预测性或抗针对程度的证明。
...
先只读核对本轮实现与 `docs/53` 的一致性；随后如需推进，**唯一建议的下一步是单独授权冻结逐张牌 fixture 清单与输出目录，并申请阶段 A 的运行许可**。阶段 A 实测披露之后，再另行裁定阶段 B 预算可行性与质量签收；默认切换必须晚于质量签收并单独授权。
