...
- [phase6/74 新身份研发与独立验证战役章程](phase6/74-m8-new-identity-and-independent-verification-campaign-charter.md)：当前入口；登记 `74` 已开启及其范围、前置与未授权边界；**不构成实现、运行或质量结论**。
- [phase6/73f `73` 战役关闭记录（未冻结）](phase6/73f-m8-campaign-closure-without-framework-freeze-record.md)：在 `73b` 保持未冻结的前提下关闭 `73`，`73` 后缀不得续用；**不等于冻结、运行或质量结论**。
...
| [`phase6/`](phase6/) | 55–58n、60–73f、74 | 独立质量验证、可诊断性升级、签收与只读诊断；`73` 为 v10 后的质量框架与路线决策（已由 `73f` 在未冻结前提下关闭），`74` 为新身份研发与独立验证战役（已开启）。 |
...
- 2026-09-24：经单独授权开启新根编号 `74`，新增 `phase6/74` 号「新身份研发与独立验证战役章程」（本战役根文档），只登记 `74` 已开启及其范围、前置与未授权边界；不构成实现、运行、验证或冻结授权，不改代码、不创建身份、不生成或冻结 seed / 节点 / 清单、不提交推送；`73` 族保持关闭且不得续号；`phase6/` 编号范围更新为 `55–58n、60–73f、74`。
