...
>
> 本文件**分两次提交**，沿用 `docs/33` / `docs/33a` 的"规格与回执分开"惯例：第一次随修复代码提交，只写第 1 至第 9 节的决策与冻结规格；第二次在执行完成后另写 `docs/34a` 记录实际回执，本文件只补一处指向更正。这样写是为了让"冻结提交"早于任何受监督运行，避免文档自身改动打断工作树干净前置。
>
> **第 1 至第 9 节提交时，跨 seed campaign 尚未启动，不声称已生成任何新策略、质量或资源证据。** `docs/20` 至 `docs/33c` 未被改写。
>
> 实际执行回执、封存清单与事前注册口径下的跨 seed 对照结果见 `docs/34a-m8-multiplayer-cfr-candidate-a-cross-seed-campaign-execution-record.md`。
