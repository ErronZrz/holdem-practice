...
## 11. macOS 外部 supervisor 原语（未监督实际训练）

本轮新增 `supervisor.py`，实现仅用于独立离线受控子进程的 macOS 原语，不执行 MCCFR 训练命令：

1. 只接受非空参数数组，明确禁用 shell；工作目录和工件根目录必须是现有非链接目录。
2. 受控子进程在新 session/进程组启动；固定调用 `/bin/ps` 的 C locale 快照，按父子关系追踪当前进程树，记录累计 CPU、当前/峰值 RSS 和观察到的 PID。
3. 所有已观察进程的 CPU 最大值按 PID 累计；墙钟、CPU、RSS 预警/硬停、工件目录总字节和单并发限制均由父监督器比较。工件根目录出现链接或非普通文件时立即视为监督失败。
