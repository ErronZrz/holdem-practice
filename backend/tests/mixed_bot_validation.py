...
# 第六套主种子：再换独立派生前缀；块数与前一套相同，只为换取新样本。
MIXED_IQV5_SEED_PREFIX = "mixed-local-iqv-v5:seed:"
MIXED_IQV5_MAIN_SEED_COUNT = 16


def derive_iqv5_main_seeds(count: int = MIXED_IQV5_MAIN_SEED_COUNT) -> tuple[int, ...]:
    """按冻结规则复算第六套主种子：派生串的 SHA-256 前 4 字节按大端解释为无符号整数。"""
    if count < 1:
        raise MixedValidationAuthorizationError("主种子个数必须为正")
    return tuple(
        int.from_bytes(
            hashlib.sha256(f"{MIXED_IQV5_SEED_PREFIX}{index}".encode()).digest()[:4],
            "big",
        )
        for index in range(1, count + 1)
    )
...
MIXED_IQV5_MAIN_SEEDS: tuple[int, ...] = (
    3008262424,
    1713517322,
    1076959820,
    2076521261,
...
# 第六套清单的机械明细文本：节点与种子再次更换，因此场景顺序与手数说明同步重写。
MIXED_IQV5_SCENARIO_ORDER = (
    "按第六套配方类别顺序 × 适用人数升序；成本矩阵每格在该街的可用节点间顺序轮换"
)
MIXED_IQV5_STAGE_PLAN: tuple[str, ...] = (
    "阶段 A：成本矩阵逐人数 36 格各取首个样本，"
    f"合计 {STAGE_A_COST_SAMPLES} 次；补充场景首样本 {STAGE_A_SUPPLEMENTARY_SAMPLES} 次；"
    f"对抗对照 {STAGE_A_ADVERSARIAL_HANDS_IQV2} 手（单一主种子、单一风格、单一轮换）",
    f"阶段 B：成本矩阵补足逐人数 {COST_DECISIONS_PER_PLAYER_COUNT} 次、"
...
def frozen_iqv5_manifest(
    *,
    code_identity: str,
    output_dir: str,
    strategy_id: str,
) -> MixedFixtureManifest:
    """装配第六套冻结清单：又一套节点集与主种子，报告版本与前一版相同。
    ...
    return build_frozen_manifest(
        ...
        code_identity=code_identity,
        config_digest=config_digest(strategy_id),
        seeds=MIXED_IQV5_MAIN_SEEDS,
        nodes=build_frozen_iqv5_nodes(),
        output_dir=output_dir,
        scenario_order=MIXED_IQV5_SCENARIO_ORDER,
        seed_derivation=MIXED_SEED_DERIVATION,
        stage_plan=MIXED_IQV5_STAGE_PLAN,
        resource_envelope=MIXED_RESOURCE_ENVELOPE,
        stop_conditions=MIXED_STOP_CONDITIONS,
        report_format=MIXED_REPORT_FORMAT_V2,
    )
...
def _rebuild_fixture(
    ...
    *,
    ...
    elif fixture.node_id == base + MIXED_IQV5_NODE_ID_SUFFIX:
        builder = build_iqv5_node
    else:
        raise MixedValidationAuthorizationError(
