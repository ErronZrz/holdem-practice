"""第五套样本集的回归：节点正交性、类别语义、种子正交性与 opt-in 入口链。
...
from app.strategy.mixed_strategy import MIXED_STRATEGY_IDENTIFIER_V7
...
from .mixed_bot_validation import (
    ...
    derive_iqv4_main_seeds,
    fixture_at_depth,
    frozen_iqv4_manifest,
    validation_limitations,
)

FIFTH_SET = build_frozen_iqv4_nodes()
EARLIER_SETS = {
...
def test_fifth_set_has_the_frozen_size_and_suffix() -> None:
    assert len(FIFTH_SET) == MIXED_APPLICABLE_NODE_COUNT
    assert all(node.node_id.endswith(MIXED_IQV4_NODE_ID_SUFFIX) for node in FIFTH_SET)
    assert MIXED_IQV4_NODE_ID_SUFFIX not in EARLIER_SUFFIXES


def test_fifth_set_covers_every_planned_slot() -> None:
    pairs = {(node.category, node.player_count) for node in FIFTH_SET}
    ...
    for count in MIXED_PLAYER_COUNTS:
        streets = {node.decision_street for node in FIFTH_SET if node.player_count == count}
        assert streets == set(MIXED_STREETS)
    for node in FIFTH_SET:
        assert node.decision_street is MIXED_CATEGORY_STREET[node.category]


@pytest.mark.parametrize("name", sorted(EARLIER_SETS))
def test_fifth_set_reuses_no_earlier_node(name: str) -> None:
    """与既有各套必须正交：标识、主题牌面与行动线都不重合。"""
    earlier = EARLIER_SETS[name]
    assert not {node.node_id for node in FIFTH_SET} & {node.node_id for node in earlier}
    ours, theirs = _themes(FIFTH_SET), _themes(earlier)
    ...
    assert not combined(FIFTH_SET) & combined(earlier)


def test_fifth_set_cards_are_disjoint_and_deterministic() -> None:
    for node in FIFTH_SET:
        cards = [*node.board, *(token for pair in node.hole_cards for token in pair)]
        assert len(set(cards)) == len(cards), node.node_id
    again = build_frozen_iqv4_nodes()
    assert [node.model_dump() for node in again] == [
        node.model_dump() for node in FIFTH_SET
    ]


def test_fifth_set_replays_cleanly_and_keeps_category_semantics() -> None:
...
def test_fifth_set_themes_match_their_category_semantics() -> None:
...
def test_fifth_set_nodes_rebuild_at_other_depths() -> None:
    scalable = next(node for node in FIFTH_SET if node.depth_scalable)
    ...
    fixed = next(node for node in FIFTH_SET if not node.depth_scalable)
...
def test_fifth_set_seeds_are_derived_and_disjoint() -> None:
    assert derive_iqv4_main_seeds() == MIXED_IQV4_MAIN_SEEDS
    assert len(MIXED_IQV4_MAIN_SEEDS) == 16
    assert len(set(MIXED_IQV4_MAIN_SEEDS)) == 16
    for earlier in (
        MIXED_IQV3_MAIN_SEEDS,
        MIXED_IQV2_MAIN_SEEDS,
        MIXED_IQV_MAIN_SEEDS,
        ...
        assert not set(MIXED_IQV4_MAIN_SEEDS) & set(earlier)


def test_fifth_manifest_carries_the_seventh_identity() -> None:
    manifest = frozen_iqv4_manifest(
        code_identity="0" * 40,
        output_dir="/tmp/unused/runs",
        strategy_id=MIXED_STRATEGY_IDENTIFIER_V7,
    )
    assert manifest.strategy_id == MIXED_STRATEGY_IDENTIFIER_V7
    assert manifest.seeds == MIXED_IQV4_MAIN_SEEDS
    assert manifest.nodes == FIFTH_SET
    assert manifest.report_format.startswith(MIXED_VALIDATION_SCHEMA_VERSION_V2)
    assert manifest.config_digest == validation.config_digest(MIXED_STRATEGY_IDENTIFIER_V7)
    assert not validation.required_envelope_fields(manifest)


def test_fifth_set_block_count_reuses_the_registered_limitations() -> None:
    """局限说明按种子块个数取：第五套与前一版块数相同，因此沿用同一条登记文本。"""
    manifest = frozen_iqv4_manifest(
        code_identity="0" * 40,
        output_dir="/tmp/unused/runs",
        strategy_id=MIXED_STRATEGY_IDENTIFIER_V7,
...
def test_fifth_set_runs_through_stage_a_and_the_recheck_entry(tmp_path: Path) -> None:
    """两条 opt-in 入口链必须真正走通：阶段 A 与小规模补算都按第七版身份执行。"""
    full = frozen_iqv4_manifest(
        code_identity="0" * 40,
    ...
    assert receipt["strategy_id"] == MIXED_STRATEGY_IDENTIFIER_V7
    ...
    assert all(row["node_id"].endswith(MIXED_IQV4_NODE_ID_SUFFIX) for row in rows)
