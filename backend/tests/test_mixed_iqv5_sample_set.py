"""第六套样本集的回归：节点正交性、类别语义、种子正交性与 opt-in 入口链。
...
from app.strategy.mixed_strategy import MIXED_STRATEGY_IDENTIFIER_V8
...
from .mixed_bot_validation import (
    ...
    MIXED_IQV4_MAIN_SEEDS,
    MIXED_IQV5_MAIN_SEEDS,
    MIXED_IQV_MAIN_SEEDS,
    MIXED_MAIN_SEEDS,
    MIXED_VALIDATION_SCHEMA_VERSION_V2,
    derive_iqv5_main_seeds,
    fixture_at_depth,
    frozen_iqv5_manifest,
    validation_limitations,
)

SIXTH_SET = build_frozen_iqv5_nodes()
EARLIER_SETS = {
    "首套": build_frozen_nodes(),
    "第二套": build_frozen_iqv_nodes(),
    "第三套": build_frozen_iqv2_nodes(),
    "第四套": build_frozen_iqv3_nodes(),
    "第五套": build_frozen_iqv4_nodes(),
}
EARLIER_SUFFIXES = (
    MIXED_IQV_NODE_ID_SUFFIX,
    MIXED_IQV2_NODE_ID_SUFFIX,
...
# 已永久消耗的标定授权所用种子：新样本集必须与它们零交集。
CONSUMED_CALIBRATION_SEEDS = (1215, 20260918, 3311, 7926)
...
def test_sixth_set_has_the_frozen_size_and_suffix() -> None:
    assert len(SIXTH_SET) == MIXED_APPLICABLE_NODE_COUNT
    assert all(node.node_id.endswith(MIXED_IQV5_NODE_ID_SUFFIX) for node in SIXTH_SET)
    assert MIXED_IQV5_NODE_ID_SUFFIX not in EARLIER_SUFFIXES


def test_sixth_set_covers_every_planned_slot() -> None:
    pairs = {(node.category, node.player_count) for node in SIXTH_SET}
    ...
    for count in MIXED_PLAYER_COUNTS:
        streets = {node.decision_street for node in SIXTH_SET if node.player_count == count}
        assert streets == set(MIXED_STREETS)
    for node in SIXTH_SET:
        assert node.decision_street is MIXED_CATEGORY_STREET[node.category]


@pytest.mark.parametrize("name", sorted(EARLIER_SETS))
def test_sixth_set_reuses_no_earlier_node(name: str) -> None:
    """与既有各套必须正交：标识、主题牌面与行动线都不重合。"""
    earlier = EARLIER_SETS[name]
    assert not {node.node_id for node in SIXTH_SET} & {node.node_id for node in earlier}
    ours, theirs = _themes(SIXTH_SET), _themes(earlier)
    ...
    assert not combined(SIXTH_SET) & combined(earlier)


def test_sixth_set_cards_are_disjoint_and_deterministic() -> None:
    for node in SIXTH_SET:
        cards = [*node.board, *(token for pair in node.hole_cards for token in pair)]
        assert len(set(cards)) == len(cards), node.node_id
    again = build_frozen_iqv5_nodes()
    assert [node.model_dump() for node in again] == [
        node.model_dump() for node in SIXTH_SET
    ]


def test_sixth_set_replays_cleanly_and_keeps_category_semantics() -> None:
...
def test_sixth_set_themes_match_their_category_semantics() -> None:
    """主题牌面必须真的构成该类别声称的局面；只断言性质，不写任何门槛数值。

    牌面与底牌以「点数 + 花色」两个字符的记号存储，因此点数可直接按首字符比较。
    """
    for node in SIXTH_SET:
        applied = apply_node(node)
        engine = applied.engine
        seat = engine.current_seat
        cards = engine.players[seat].hole_cards
        ranks = [token[0] for token in node.hole_cards[seat]]
        board_ranks = {token[0] for token in node.board}
        if node.category in ("flop-draw", "turn-combo-draw"):
            features = extract_features(applied.actor_state(), applied.legal_actions())
            assert features.draw > 0, node.node_id
        elif node.category == "top-pair-weak-kicker":
            hero = evaluate_fast([*cards, *engine.board])
            assert hero.category is HandCategory.ONE_PAIR, node.node_id
            assert ranks[0] in board_ranks or ranks[1] in board_ranks, node.node_id
        elif node.category == "overpair-on-high-board":
            assert ranks[0] == ranks[1], node.node_id
        elif node.category == "missed-draw-river":
            hero = evaluate_fast([*cards, *engine.board])
            assert hero.category is HandCategory.HIGH_CARD, node.node_id
        elif node.category == "sizes-merged":
            hero = evaluate_fast([*cards, *engine.board])
            assert hero.category is HandCategory.ONE_PAIR, node.node_id


def test_sixth_set_nodes_rebuild_at_other_depths() -> None:
    scalable = next(node for node in SIXTH_SET if node.depth_scalable)
    ...
    fixed = next(node for node in SIXTH_SET if not node.depth_scalable)
...
def test_sixth_set_seeds_are_derived_and_disjoint() -> None:
    """新盐派生结果必须可复算、互不重复，且与既有五套及已消耗标定种子零交集。"""
    assert derive_iqv5_main_seeds() == MIXED_IQV5_MAIN_SEEDS
    assert len(MIXED_IQV5_MAIN_SEEDS) == 16
    assert len(set(MIXED_IQV5_MAIN_SEEDS)) == 16
    for earlier in (
        MIXED_IQV4_MAIN_SEEDS,
        MIXED_IQV3_MAIN_SEEDS,
        MIXED_IQV2_MAIN_SEEDS,
        ...
        assert not set(MIXED_IQV5_MAIN_SEEDS) & set(earlier)


def test_sixth_manifest_carries_the_eighth_identity() -> None:
    manifest = frozen_iqv5_manifest(
        code_identity="0" * 40,
        output_dir="/tmp/unused/runs",
        strategy_id=MIXED_STRATEGY_IDENTIFIER_V8,
    )
    assert manifest.strategy_id == MIXED_STRATEGY_IDENTIFIER_V8
    assert manifest.seeds == MIXED_IQV5_MAIN_SEEDS
    assert manifest.nodes == SIXTH_SET
    assert manifest.report_format.startswith(MIXED_VALIDATION_SCHEMA_VERSION_V2)
    assert manifest.config_digest == validation.config_digest(MIXED_STRATEGY_IDENTIFIER_V8)
    assert not validation.required_envelope_fields(manifest)


def test_sixth_set_block_count_reuses_the_registered_limitations() -> None:
    """局限说明按种子块个数取：第六套与前一版块数相同，因此沿用同一条登记文本。"""
    manifest = frozen_iqv5_manifest(
        code_identity="0" * 40,
        output_dir="/tmp/unused/runs",
        strategy_id=MIXED_STRATEGY_IDENTIFIER_V8,
...
def test_sixth_set_runs_through_stage_a_and_the_recheck_entry(tmp_path: Path) -> None:
    """两条 opt-in 入口链必须真正走通：阶段 A 与小规模补算都按第八版身份执行。"""
    full = frozen_iqv5_manifest(
        code_identity="0" * 40,
    ...
    assert receipt["strategy_id"] == MIXED_STRATEGY_IDENTIFIER_V8
    ...
    assert all(row["node_id"].endswith(MIXED_IQV5_NODE_ID_SUFFIX) for row in rows)
